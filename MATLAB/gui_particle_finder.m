...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Alex O, Sept 8, 2009 Version 1.5.6
%fixed spot adding, removal over switched area selections in frame
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Alex O, Sept 11, 2009 Version 1.5.7
%fixed track filtering, also now slightly faster
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Alex O, Sept 24, 2009 Version 1.6
%added mouseover comments, made several minor changes
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Alex O, Oct 15, 2009 Version 1.8
%fixed some mouseover comments, started adding drift correction (now
%invisible) also did documentation and fixed minor bug in movie_slider
%Alex O, Jan 8, 2010 Version 1.9
%Should be the final version, this was tested and has all changes requested
%by various people including, but not limited to, various bug fixes, requested
%saving features, more tooltips and revisions of previous tooltips as well
%as a help button that opens an html file with a detailed explanation of
%every part of the gui.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Alex O, Aug 6, 2010 Version 1.9.1
%now has border drawn for masks, also allows the loading of masks
...
[bw x_line y_line]=masking(handles);
handles.bw=bw;
handles.x_line=x_line;
handles.y_line=y_line;
handles.tracks=[];
handles.aoi_add=[];
handles.aoi_remove=[];
handles.selected=[];
...
if view_aois_tracks==1 %view aois, frame by frame options
    ...
    if ~isempty(handles.aoi_remove) %will remove peaks from pk
        ...
        if ~isempty(handles.aoi_add)
            ...
            %set removeFrom to location relative to current frame
            %boundaries
            removeFrom=[removeFrom(:,1)-area_selected(1,1)+1 removeFrom(:,2)-area_selected(1,3)+1];
        end
            ...
            if ~isempty(toRemove)
                ...
                for i=toRemove_length:-1:1
                    ...
                    if min(store)<min(store_aoi_add) %pk has closer point
                        ...
                        aoi_add_search=handles.aoi_add(:,1:2);
                        aoi_add_search=[aoi_add_search(:,1)-area_selected(1,1)+1 aoi_add_search(:,2)-area_selected(1,3)+1];
                        [a ind]=ismember(removeFrom(find(store_aoi_add==min(store_aoi_add)),:),aoi_add_search,'rows');
    ...
    if ~isempty(handles.aoi_add) && view_aois_tracks==1 %will add peaks to pk
        ...
        %set toAdd to location relative to current frame
        %boundaries
        toAdd=[toAdd(:,1)-area_selected(1,1)+1 toAdd(:,2)-area_selected(1,3)+1];
        if ~isempty(toAdd)
            pk=[pk;toAdd];
        end
    end  
...
if ~isempty(pk)
    ...
    hold on;
    line(handles.x_line,handles.y_line);
    pksize=size(pk);
    ...
    for i=pksize(1,1):-1:1
        %handles.bw(area_selected(1,3):area_selected(1,4),area_selected(1,1
        %):area_selected(1,2))
        %[pk(i,2) pk(i,1)]
        if handles.bw(area_selected(1,3)+floor(pk(i,2))-1,area_selected(1,1)+floor(pk(i,1))-1)==0
    ...
    if ~isempty(pk)
        ...
        if view_aois_tracks==3 && ~isempty(handles.tracks)
            ...
            ColorSet=varycolor(track_id_size(1)+10);
...
if ~isempty(handles.tracks)
    ...
    cur_frame_tracks=handles.tracks(find(handles.tracks(:,3)==selected_frame),1:4);
    [cur_frame_tracks_size useless]=size(cur_frame_tracks);
    track_count=0;
    for i=cur_frame_tracks_size:-1:1
        if handles.bw(area_selected(1,3)+floor(cur_frame_tracks(i,2))-1,area_selected(1,1)+floor(cur_frame_tracks(i,1))-1)==1
...
function [mask x_line y_line]=masking(handles)
path=get(handles.file_path,'string');
...
if get(handles.track_edit_button,'value') == 0 %off (default value), make add/remove aoi panel invisible
    ...
    set(handles.drift_corr_button,'value',0)
    drift_corr_button_Callback(hObject, eventdata, handles);
    set(handles.trajectory_info_panel,'visible','on');
end

% --- Executes on button press in drift_corr_button.
function drift_corr_button_Callback(hObject, eventdata, handles)
if get(handles.drift_corr_button,'value') == 0
    set(handles.drift_corr_panel,'visible','off');
    set(handles.trajectory_info_panel,'visible','off');
else %on, make add/remove aoi panel visible
    view_type=get(handles.view_aois_tracks,'value');
    if view_type<2 %if not view tracks or view colored tracks
    ...
    set(handles.drift_corr_panel,'visible','on');
    set(handles.spot_edit_button,'value',0)
...
% Hint: get(hObject,'Value') returns toggle state of drift_corr_button
...
if isempty(tracks_in_frame)
    ...
    set(handles.disp1,'value',track_id);
...
fpath=get(handles.file_path,'string');
frame_average=str2num(get(handles.frame_average,'string'));
diam_pix=str2num(get(handles.size,'string'));
save_path=[get(handles.file_path,'string') 'frames-' num2str(slider_min) '-' num2str(slider_max) '-noise-' get(handles.noise,'string') '-size-' get(handles.size,'string') '-bright-' get(handles.pk_find,'string') '.xls'];
switch to_output %case 1 is 'do nothing'
    ...
    case 3 %graph spots vs frames
        ...
        ylim([0 max(spots_per_frame(:,2))]);
        xlabel('Frame');
        ylabel('Spots in Frame');
        [filename pathname]=uiputfile(' ','save info as','info.mat');
        save([pathname filename],'spots_per_frame');
        [SUCCESS]=xlswrite(save_path,spots_per_frame);
        if SUCCESS==false
           'failed to write spots per frame data to excel file!' 
        end
    ...
    case 4 %graph spots vs time
        ...
        for i=slider_min:slider_max
            ...
            frame_select_Callback(hObject, eventdata, handles);
            point_count=str2num(get(handles.points_found,'string'));
            spots_per_time=[spots_per_time;vid.ttb(i) point_count];
        end
        spots_per_time
        
        plot(spots_per_time(:,1),spots_per_time(:,2));
        ylim([0 max(spots_per_time(:,2))]);
        xlim([min(spots_per_time(:,1)) max(spots_per_time(:,1))]);
        xlabel('Time');
        ylabel('Spots in Frame');
        [filename pathname]=uiputfile(' ','save info as','info.mat');
        save([pathname filename],'spots_per_time');
        [SUCCESS]=xlswrite(save_path,spots_per_time);
        if SUCCESS==false
           'failed to write spots per time data to excel file!' 
        end
    ...
    case 5 %save coordinates in frame
        ...
        %now follows larry's imscroll import requirements
        coord=[ones(length(coord),1)*selected_frame ones(length(coord),1)*frame_average coord(:,1:2) ones(length(coord),1)*diam_pix rot90([1:length(coord)],3)]; %#ok<NASGU>
        [filename pathname]=uiputfile(' ','save info as','info.mat');
        save([pathname filename],'coord');
        save_path_coord=[get(handles.file_path,'string') 'coord-in-frame-' get(handles.frame_select,'string') '-noise-' get(handles.noise,'string') '-size-' get(handles.size,'string') '-bright-' get(handles.pk_find,'string') '.mat'];
        save (save_path_coord,'coord');
        [SUCCESS]=xlswrite(save_path_coord,coord);
        if SUCCESS==false
           'failed to write locations in current frame data to excel file!' 
        end
    ...
    case 6 %save coordinates for all frames
        ...
        for i=slider_min:slider_max
            ...
            if ~isempty(coord)
                ...
                coord_all=[coord_all;[ones(length(coord),1)*i coord(:,1:2) ones(length(coord),1)*frame_average ones(length(coord),1)*diam_pix]];
                %now works with larry's imscroll program
                %coord=[ones(length(coord),1)*i ones(length(coord),1)*frame_average coord(:,1:2) ones(length(coord),1)*diam_pix rot90([1:length(coord)],3)]; %#ok<NASGU>
            end
        end
        coord_all=[coord_all rot90([1:size(coord_all,1)],3)];
        [filename pathname]=uiputfile(' ','save info as','info.mat');
        save([pathname filename],'coord_all');
        save_path_coords=[get(handles.file_path,'string') 'coordsinframes-' num2str(slider_min) '-' num2str(slider_max) '-noise-' get(handles.noise,'string') '-size-' get(handles.size,'string') '-bright-' get(handles.pk_find,'string') '.xls'];
        save (save_path,'coord_all');
        [SUCCESS]=xlswrite(save_path_coords,coord_all);
        if SUCCESS==false
           'failed to write all locations in all frames data to excel file!' 
        end
        
    case 7 %graph tracks vs frames
        if get(handles.view_aois_tracks,'value')==1
            set(handles.view_aois_tracks,'value',2);
        end
        if isfield(handles,'tracks')
            if ~isempty(handles.tracks)
                ...
                if ~isempty(tracks_in_frame)
                    ...
                    ylim([0 max(tracks_in_frame(:,2))]);
                    xlabel('Frames');
                    ylabel('Tracks in Frame');
                    [filename pathname]=uiputfile(' ','save info as','info.mat');
                    save([pathname filename],'tracks_in_frame');
                    [SUCCESS]=xlswrite(save_path,tracks_in_frame);
                    if SUCCESS==false
                       'failed to write tracks vs frames data to excel file!' 
                    end
                else
        ...
        if isfield(handles,'tracks')
            if ~isempty(handles.tracks)
                ...
                if ~isempty(cur_frame_tracks)
                    ...
                    ylim([0 max(cumulative_tracks(:,2))]);
                    xlabel('Frames');
                    ylabel('Cumulative Tracks');
                    [filename pathname]=uiputfile(' ','save info as','info.mat');
                    save([pathname filename],'cumulative_tracks');
                    [SUCCESS]=xlswrite(save_path,cumulative_tracks);
                    if SUCCESS==false
                       'failed to write cumulative tracks data to excel file!' 
                    end
                else
                    'no tracks in the current frame range!'
                end
...
selected_frame=str2num(get(handles.frame_select,'string'));
[xin yin button]=ginput(1);
%add selected area size so it is theabsolute location of the point in the
%frame
xin=xin+area_selected(1,1)-1;
yin=yin+area_selected(1,3)-1;
...
% --- Executes on button press in textbx_213213.
function track_info_Callback(hObject, eventdata, handles)
view_type=get(handles.view_aois_tracks,'value');
if view_type<2 %if not view tracks or view colored tracks
        set(handles.view_aois_tracks,'value',2); %set to view tracks
        view_aois_tracks_Callback(hObject, eventdata, handles);
...
% hObject    handle to textbx_213213 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in ref_frame_track_button.
function ref_frame_track_button_Callback(hObject, eventdata, handles)
track_id=get(handles.disp1,'value');
handles.selected=[handles.selected track_id];
frames=unique(str2num(get(handles.drift_corr_frames,'string')));
frames_check=[frames;zeros(1,size(frames,2))];
for i=handles.selected
   selected_track=handles.tracks(find(handles.tracks(:,4)==i),1:4);
   for j=frames
       if ~isempty(find(selected_track(:,3)==j, 1))
           frames_check(2,j)=1;
       end
   end
end
covered=sum(frames_check(2,:))/size(frames_check,2)*100
set(handles.drift_corr_percent_covered,'string',[num2str(covered) ' % Covered']);
guidata(gcbo,handles);
% hObject    handle to ref_frame_track_button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
function drift_corr_frames_Callback(hObject, eventdata, handles)
frames=str2num(get(handles.drift_corr_frames,'string'))
frames(1,2)
% hObject    handle to drift_corr_frames (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of drift_corr_frames as text
%        str2double(get(hObject,'String')) returns contents of drift_corr_frames as a double
...
% --- Executes on button press in help_button.
function load_mask_button_Callback(hObject, eventdata, handles)
[filename, pathname] = uigetfile('*.mat', 'Choose a mask file');
load([pathname filename]);
handles.bw=mask_files.mask;
handles.x_line=mask_files.x;
handles.y_line=mask_files.y;
set(handles.manual_mask,'value',1);
guidata(gcbo,handles);
movie_slider_Callback(hObject, eventdata, handles);
% hObject    handle to help_button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
