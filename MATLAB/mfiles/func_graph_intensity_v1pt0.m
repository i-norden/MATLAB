...
function graph=func_graph_intensity_v1pt0(file_path,header_path,min_frame,max_frame,spot_number,type)
...
%           'minusbackground'= aoifits.data with background subtracted
%           'detrendedtrace'= Intervals.Intervals.AllTracesCellArray(:,12)
%           'inputtrace'= Intervals.Intervals.AllTracesCellArray(:,11)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%V.1.0  Alex O   3/9/10
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%info
%V.1.1  Alex O   3/23/10
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%now has detrendedtrace and inputtrace cases
%V.1.0  Alex O   3/9/10
...
switch lower(type)
    ...
    case 'minusbackground'
        ...
        if ~isfield(aoifits,'BackgroundData') %if old version of aoifits
            ...
            graph=[];
            return
        elseif size(aoifits.data)~=size(aoifits.BackgroundData) %different dimentions
            'Wrong dimentions for BackgroundData field, cannot subtract background'
            graph=[];
            return
    ...
    case 'inputtrace'
        for i=1:size(Intervals.AllTracesCellArray(:,11)) %only by actual size of spots, request may be more so I do it this way
            peaks=Intervals.AllTracesCellArray{i,11};
            for j=1:size(peaks,1)
                if peaks(j,1)>=min_frame && peaks(j,1)<=max_frame %if within range
                    graph(i,peaks(j,1)-min_frame+1)=peaks(j,2);
                end
