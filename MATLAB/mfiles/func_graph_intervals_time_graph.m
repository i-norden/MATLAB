function func_graph_intervals_time_graph(graph,time2frames,sneg,s0,s1,s2,s3,cneg,c0,c1,c2,c3)
    ...
    for j=2:size(graph,2) %for each element of each row of graph
        if graph(i,j)==curr %current value still in place
            ...
            if curr==sneg %-1 No data
                rectangle('Position',[xpos,i-1,width,1],'EdgeColor',cneg,'FaceColor',cneg);
            elseif curr==s0 %0 No peaks
                rectangle('Position',[xpos,i-1,width,1],'EdgeColor',c0,'FaceColor',c0);   
            elseif curr==s1 %1 %Peak on file2
                rectangle('Position',[xpos,i-1,width,1],'EdgeColor',c1,'FaceColor',c1);                
            elseif curr==s2% 2 %peak on file1
                rectangle('Position',[xpos,i-1,width,1],'EdgeColor',c2,'FaceColor',c2);
            elseif curr==s3% 3 %peak on both files (or just peak if only 1 file)
                rectangle('Position',[xpos,i-1,width,1],'EdgeColor',c3,'FaceColor',c3);
            else
