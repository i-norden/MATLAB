...
%progress_bar_param== added by Alex to add progress bar to this function
...
for frmindx=frmrange(1):frmrange(1)+averange-1
    ...
    %size(fullframe)
    if frmlim(2)>size(fullframe,1)
        frmlim(2)=size(fullframe,1);
    end
    if frmlim(4)>size(fullframe,2)
        frmlim(4)=size(fullframe,2);
    end
    %frmlim
    %size(dum1(:,:,indx))
    %size(fullframe(frmlim(1):frmlim(2),frmlim(3):frmlim(4)))
...
for frmindx=(frmrange(1)+averange):averange:frmrange(2)
    ...
    for subfrmindx=frmindx:frmindx+averange-1
        ...
        indx=indx+1;
        subfunc_progress_bar(progress_bar_param.handles,subfrmindx,1,progress_bar_param.frames_total,1);
    end
