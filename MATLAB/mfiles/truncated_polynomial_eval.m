function pc=truncated_polynomial_eval(arg,xinput)
%
% function truncated_polynomial_eval(arg,xinput)
%
% To be used in conjuction with the Matlab function 'fminsearch' in
% order to fit a exponential function of the from:
%
...
%pc= sum((rawdata(:,2)-(arg(1)*exp(-(rawdata(:,1)-arg(2) )/arg(3) )   +arg(4)) ).^2);
                    ...
                    % simpler form:
x=xinput;
pc= arg(1)*x + arg(2)*x.^2 + arg(3)* x.^3 + arg(4)*x.^4+arg(5)*x.^5 + arg(6)*x.^6 + arg(7)*x.^7 + arg(8)*x.^8;
