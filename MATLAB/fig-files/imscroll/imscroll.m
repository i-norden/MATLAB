...
% IMSCROLL Application M-file for imscrollmou.fig
%    FIG = IMSCROLL launch imscroll GUI.
%    IMSCROLL('callback_name', ...) invoke the named callback.

% Last Modified by GUIDE v2.5 12-May-2012 19:26:05
...
%                              (pixnum-1)/2,'b')
    ...
    if get(handles.MarkFolder2SpotsToggle,'Value')==1   % Test if toggle is depressed
        MarkFolder2Spots(handles);          % Here to mark the spots that appeared in the
                                            % folder2 sequence
    end    
...
%magrange_string=['(' num2str(ypts(1)) ':' num2str(ypts(2)) ',' num2str(xpts(1)) ':' num2str(xpts(2)) ')'];
if get(handles.MagChoice,'Value')==0
                % Here to write and store field limits for case 0
     magrange_string=['[' num2str(xpts(1)) ' ' num2str(xpts(2)) ' ' num2str(ypts(1)) ' ' num2str(ypts(2)) ']'];
     set(handles.MagRangeYX,'String',magrange_string);
     userdat=get(handles.MagChoice,'UserData');
     userdat(1,:)=[xpts(1) xpts(2) ypts(1) ypts(2)];
     set(handles.MagChoice,'UserData',userdat);
else
                % Here to write and store field limits for case 1
     magrange_string=['[' num2str(xpts(1)) ' ' num2str(xpts(2)) ' ' num2str(ypts(1)) ' ' num2str(ypts(2)) ']'];
     set(handles.MagRangeYX,'String',magrange_string);
     userdat=get(handles.MagChoice,'UserData');
     userdat(2,:)=[xpts(1) xpts(2) ypts(1) ypts(2)];
     set(handles.MagChoice,'UserData',userdat);
end
% --- Executes during object creation, after setting all properties.
...
elseif argnum==3                                        % map the present AOI set to 
 ...
 %handles.FitData(:,4)=handles.FitData(:,4)*fitparmvector(3) + fitparmvector(4);
         if get(handles.ProximityMappingToggle,'Value')==1
             ...
             mappednow(indx,:)=proximity_mapping(handles.MappingPoints,nowpts(indx,:),15,fitparmvector,2);
             end
...
elseif argnum==4                                        % Invert the map: go from e.g. x2 -> x1
     ...
     if get(handles.ProximityMappingToggle,'Value')==1
             ...
             mappednow(indx,:)=proximity_mapping(handles.MappingPoints,nowpts(indx,:),15,fitparmvector,1);
             end
             handles.FitData(:,3:4)=mappednow;
         end
...
while flag==0
    ...
    if but==3
        ...
        log=(handles.FitData(:,6)==num_closest);
       
        handles.FitData(log,:)=[];          % remove information for that aoi
                                            % Refresh display
...
userdat=get(handles.MagChoice,'UserData');          % Retrieve the UserData matrix containing the sets of xy limits
                                                    % userdat = [xpts(1) xpts(2) ypts(1) ypts(2)]
                                                    
% Change the button display and alter the MagRangeYX
if get(handles.MagChoice,'Value')==0
    set(handles.MagChoice,'String','0')
    % retrieve the xy limits from UserData matrix, and place in text field
    % (which is used to set display when the Magnify button is pushed)
    magrange_string=['[' num2str(userdat(1,1)) ' ' num2str(userdat(1,2)) ' ' num2str(userdat(1,3)) ' ' num2str(userdat(1,4)) ']'];
    set(handles.MagRangeYX,'String',magrange_string);
else
    set(handles.MagChoice,'String','1')
    magrange_string=['[' num2str(userdat(2,1)) ' ' num2str(userdat(2,2)) ' ' num2str(userdat(2,3)) ' ' num2str(userdat(2,4)) ']'];
    set(handles.MagRangeYX,'String',magrange_string);
end
...
if get(handles.SpotsPopup,'Value')==1
    set(handles.SpotsButton,'String','Pick')
