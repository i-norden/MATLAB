function pc=macavi_with_overlay_glimpse_v5(gfolder,frames,dispscale,flagg,frmrate,avifolder,frmave,info,driftlist,aoiinfo2a, aoiinfo2b,varargin)
%
% function macavi_with_overlay_glimpse_v5(gfolder,frames,dispscale,flagg,frmrate,avifolder,frmave,info,driftlist,aoiinfo2a,aoiinfo2b,<frmlim>)
%
...
% aoiinfo2a == aoiinfo2 matrix specifying aoi centers and frames marked for
%            use in drawing aois 
% aoiinfo2b == second aoiinfo2 matrix for aois drawn with different color
%            than aoiinfo2a (intended for drawing aois in opposite field)
% frmlim == (optional) [y1 y2 x1 x2] in order to only access the portion of
%           the frame given by frm(y1:y2,x1:x2)
%global movframes dum
% V4:  added driftlist capability for the Mumm-mann
% V5:   added capability of drawing two diff color boxes, aoiinfo2a, aoiinfo2b
%     just comment it out if you do not want it
mkavi='yes';
...
aoifits=info;
aoiinfo=aoifits.aoiinfo2;                   % [(frm # when marked) ave x y pixnum aoinum]
pixnum=aoiinfo(1,5);                        % load the vid structure in the
                                            % header.mat file
...
dum_display=double(dum_display);
   ...
   %draw_aoifits_aois_no_numbers(info,'r');  % [0 .9 0] is green 
                         % Now draw the aois ***this loop added in V4
   %for indx=1:max(aoiinfo(:,6))     % Cycle through all aois
   %    XYshift=ShiftAOI(indx,inum,aoiinfo,driftlist);   % indx=aoi index,  inum=frame number index
   %    draw_box(aoiinfo(indx,3:4)+XYshift,(pixnum)/2,(pixnum)/2,'b');
   %end
    for indx=1:max(aoiinfo2a(:,6))     % Cycle through all aois
       XYshift=ShiftAOI(indx,inum,aoiinfo2a,driftlist);   % indx=aoi index,  inum=frame number index
       draw_box(aoiinfo2a(indx,3:4)+XYshift,(pixnum)/2,(pixnum)/2,[0 0 1]);
    end
    hold on
    for indx=1:max(aoiinfo2b(:,6))     % Cycle through all aois
       XYshift=ShiftAOI(indx,inum,aoiinfo2b,driftlist);   % indx=aoi index,  inum=frame number index
       draw_box(aoiinfo2b(indx,3:4)+XYshift,(pixnum)/2,(pixnum)/2,[.8 0 0]);
    end
   hold off
%draw_aoifits_aois_no_numbers(info,[.9 0 0]); % This line removed in V4
